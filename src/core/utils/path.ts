 ...
 * 如果是运行时,则需要在启动命令的环境变量中指定_rundist才能使用所有方法
 * 否则只能使用getRootPath方法
 * 如果是使用webpack打包运行
 * 那么就算指定了_rundist,除getRootPath外其它方法也是无效的
 *
 * @export
 * @class Path
 */
export class Path {
    constructor(private readonly _rundist?: string) {}
     ...
     */
    getSrcDir() {
        return this._rundist !== undefined ? this._rundist : 'src';
    }
